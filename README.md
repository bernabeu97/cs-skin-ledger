...
> 当前版本：`v0.4.0`。项目采用 AGPL-3.0；公网部署者需向网络用户提供对应版本源代码。详见 [LICENSE](LICENSE)。
...
- 数据健康：仪表盘数据健康卡（待补填买入价 / 无行情持仓 / 估值覆盖率），一键直达账本筛选与批量补填
- UU 对账：上传平台全量记录 → 双向差异报告（平台独有可导入、系统独有待核对、金额不一致逐条确认修正）
- 月度报告：任意月份已实现盈亏、胜率、盈利/亏损 TOP5、平台分布、其他收支、当前市值，支持打印为 PDF
...
- 行情刷新并发：同平台受限并发（Steam 请求间隔 400ms），保留 CSQAQ 限流闸门
