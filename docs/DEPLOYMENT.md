...
当前方案按已确认决策开放公网 HTTP。账号密码、会话 Cookie、TOTP 验证码和业务数据在传输途中**不具备 TLS 加密保护**，因此不能把当前部署判定为“公网安全验收通过”。代码侧已提供邀请码、PBKDF2 密码哈希、TOTP 双重验证（可选，管理员不再强制）、CSRF、防暴力登录、账号隔离和审计日志；获得域名后应优先接入 HTTPS，并把 `APP_SECURE_COOKIES` 改为 `true`。
...
首次启动会按 `.env` 创建管理员。TOTP 双重验证为可选功能，可自行在「账号安全」页开启；普通账号由管理员在「实例管理」生成一次性邀请码。
